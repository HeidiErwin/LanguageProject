...
public class InferenceRule {
    ...
    public InferenceRule(IPattern[] conditions, List<IPattern>[] top, List<IPattern>[] bottom, bool isTransposable) {
    ...
    public InferenceRule(IPattern[] conditions, List<IPattern>[] top, List<IPattern>[] bottom):
        this(conditions, top, bottom, true) {}

    public InferenceRule(List<IPattern>[] top, List<IPattern>[] bottom, bool isTransposable):
        this(new IPattern[0], top, bottom, isTransposable) {}

    public InferenceRule(List<IPattern>[] top, List<IPattern>[] bottom): this(new IPattern[0], top, bottom) {}
    ...
    public List<Result> InferUpward(Model m, bool plan, List<Expression> suppositions, Expression expr) {
        return Infer(true, m, plan, suppositions, expr);
    }

    public List<Result> InferDownward(Model m, bool plan, List<Expression> suppositions, Expression expr) {
        return Infer(false, m, plan, suppositions, expr);
    }

    private List<Result> Infer(bool upward, Model m, bool plan, List<Expression> suppositions, Expression expr) {
        if (!upward) {
            UnityEngine.Debug.Log("DOWNWARD INFERENCE: TODO");
            return null;
        }
        ...
        List<Result> admissibleInferences = new List<Result>();
            ...
            foreach (IPattern matchPattern in match[i]) {
                ...
                if (bindings.Count == 0) {
                    ...
                    if (conditions == null || conditions.Length == 0) {
                        ...
                        domain = m.Find(plan, suppositions, conditions);
                    }
                    ...
                    if (domain.Count == 0) {
                        admissibleInferences.Add(new Result(conjunctPositives, conjunctNegatives, conjunctAssumptions));
                    }
                    ...
                    foreach (Dictionary<MetaVariable, Expression> binding in domain) {
                        ...
                        admissibleInferences.Add(new Result(
                ...
                foreach (Dictionary<MetaVariable, Expression> binding in bindings) {
                    ...
                    if (conditions == null || conditions.Length == 0) {
                        ...
                        domain = m.Find(plan, suppositions, boundConditions);
        ...
        if (admissibleInferences.Count == 0) {
            return null;
        }
        
        return admissibleInferences;
    }

    public InferenceRule Contrapositive() {
        ...
        return new InferenceRule(conditions, newTop, newBottom);
    }
